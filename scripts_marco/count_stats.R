...
make_main_statistic_of_sample_number <- function(raw_dir){
  ...
  all_sites <- c("TAL","KRI","BIL","POR","BAR","NAP","ATH", "VIG")
  all_files_raw <- 
  ...
  barplot <- ggplot(cnts,
    ...
    scale_fill_gradientn(colors=c("orange", "forestgreen"), limits=c(0,1), 
  ...
  sizefrac_plot <-all_files_raw %>%
    ...
    scale_fill_gradientn(colors=c("grey","blue"), limits=c(0,200))+
    theme_bw()+
    ylab("size fraction")
    ...
    cowplot::plot_grid(
      plotlist=c(barplot+theme(axis.title.x = element_blank(), 
                 ...
                 sampling_time_plot+theme(axis.title.x = element_blank(), axis.text.x = element_blank(), axis.ticks.x = element_blank(),
                                          legend.position="none"), 
                 sizefrac_plot+theme(legend.position="none")), 
      nrow=3,
      align="v",
