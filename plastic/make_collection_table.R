...
collection_table_sheet <- value_or_default(opt$collection_table_sheet, "plastic_collection_table")
local_collection_table <- value_or_default(opt$local_collection_table, "plastic_collection_table.tsv")
metadata_dir <- value_or_default(opt$metadata_dir, ".")
s3_bucket <- value_or_default(opt$s3_bucket, "s3embl/plastictest")
...
s3_root_markers <- read_lines(opt$all_s3) %>%
  ...
  mutate(
    ...
    s3_prefix=str_match(s3_raw, "^([^/]+(?:\\.ome\\.zarr|\\.zarr))$")[, 2],
    name=s3_prefix %>%
      str_remove("\\.ome\\.zarr$") %>%
      str_remove("\\.zarr$")
  ) %>%
...
if (nrow(s3_root_markers) == 0) {
  stop("No PLASTIC OME-Zarr datasets found in S3 listing.")
}
...
base_table <- s3_root_markers %>%
  ...
  mutate(
    ...
    view=coalesce(site, "plastic"),
    grid=coalesce(site, "plastic"),
