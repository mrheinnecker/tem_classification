...
usage() {
  cat <<EOF
Usage:
  bash cryo_main.sh [local|interactive|cluster] [options]
...
Options:
  ...
  --collection_table_sheet VALUE   Output sheet name, default cryo_collection_table.
  --main_dir PATH                  Base CRYO workflow directory.
  ...
  --s3_bucket VALUE                S3 bucket/prefix, default s3embl/cryotest.
...
case "$mode" in
  local)
    main_dir="${CRYO_SCREEN_DIR:-C:/projects/cryo_screen}"
...
collection_table_sheet="${COLLECTION_TABLE_SHEET:-cryo_collection_table}"
outdir="${OUTDIR:-}"
logdir="${LOGDIR:-}"
work_dir="${WORK_DIR:-$default_work_dir}"
s3_bucket="${S3_BUCKET:-s3embl/cryotest}"
...
input_table="${input_table:-${main_dir}/cryo_images.tsv}"
outdir="${outdir:-${main_dir}/processed}"
logdir="${logdir:-${main_dir}/logs/wfCRYO_${timestamp}}"
